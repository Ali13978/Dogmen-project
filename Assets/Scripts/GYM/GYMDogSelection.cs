...
public class GYMDogSelection : MonoBehaviour
{
    ...
    List<DogSO> purchasedDogs;
    public void InitGYMDog()
    {
        purchasedDogs = DogsSaveLoad.instance.FetchAllDogs();
        currentIndex = Random.Range(0, purchasedDogs.Count);
        foreach (GameObject dog in AllDogs)
        {
            dog.SetActive(false);
        }

        AllDogs[purchasedDogs[currentIndex].index].SetActive(true);

        GYMDogStatCanvas.Instance.RefreshStatsUI(AllDogsSOs[purchasedDogs[currentIndex].index]);
    ...
    public void NextDog()
    {
        ...
        currentIndex++;
        if (currentIndex >= purchasedDogs.Count)
            currentIndex = 0;

        AllDogs[purchasedDogs[currentIndex].index].SetActive(true);
        DogSO dogSO = DogsSaveLoad.instance.GetDogById(purchasedDogs[currentIndex].index);
        if (dogSO == null)
            dogSO = AllDogsSOs[purchasedDogs[currentIndex].index];

        GYMDogStatCanvas.Instance.RefreshStatsUI(dogSO);
    }
    ...
    public void PreviousDog()
    {
        ...
        currentIndex--;
        if (currentIndex < 0)
            currentIndex = purchasedDogs.Count - 1;
